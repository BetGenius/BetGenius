...
def parse_config_into_betg_config():
    ...
    config_dict["PACKAGE_STRING"] = f"\"BetGenius Core {version}\""

    with open(os.path.join(SOURCE_DIR,'../build_msvc/betgenius_config.h.in'), "r", encoding="utf8") as template_file:
    ...
    with open(os.path.join(SOURCE_DIR,'../build_msvc/betgenius_config.h'), "w", encoding="utf8") as betg_config:
        betg_config.writelines(template)
...
def main():
    parser = argparse.ArgumentParser(description='BetGenius-core msbuild configuration initialiser.')
    ...
    parse_config_into_betg_config()
    copyfile(os.path.join(SOURCE_DIR,'../build_msvc/betgenius_config.h'), os.path.join(SOURCE_DIR, 'config/betgenius-config.h'))
