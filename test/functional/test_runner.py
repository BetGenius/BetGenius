...
`test/functional/test_framework/test_framework.py:BetGeniusTestFramework.main`.
...
BASE_SCRIPTS = [
    ...
    'interface_betgenius_cli.py --legacy-wallet',
    'interface_betgenius_cli.py --descriptors',
...
def main():
    ...
    enable_betgeniusd = config["components"].getboolean("ENABLE_BETGENIUSD")

    if not enable_betgeniusd:
...
def run_tests(*, test_list, src_dir, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, use_term_control, skipunit=False):
    ...
    # Warn if betgeniusd is already running
    try:
        # pgrep exits with code zero when one or more matching processes found
        if subprocess.run(["pgrep", "-x", "betgeniusd"], stdout=subprocess.DEVNULL).returncode == 0:
            print("%sWARNING!%s There is already a betgeniusd process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    except OSError:
        # pgrep not supported
        pass
    ...
    # a hard link or a copy on any platform. See https://github.com/BetGenius/BetGenius/pull/27561.
...
class RPCCoverage():
    ...
    commands per `betgenius-cli help` (`rpc_interface.txt`).
