This directory contains integration tests that test betgeniusd and its
...
betgeniusd and betgenius-qt by interacting with them through the RPC and P2P
interfaces.
- [util](/test/util) which tests the utilities (betgenius-util, betgenius-tx, ...).
...
Before tests can be run locally, BetGenius Core must be built.  See the [building instructions](/doc#building) for help.
...
The P2P and RPC ports used by the betgeniusd nodes-under-test are chosen to make
conflicts with other processes unlikely. However, if there is another betgeniusd
process running on the system (perhaps from a previous test which hasn't successfully
killed all its betgeniusd nodes), then there may be a port conflict which will
cause the test to fail. It is recommended that you run the tests on a system
where no other betgeniusd processes are running.

On linux, the test framework will warn if there is another
betgeniusd process running when the tests are started.

If there are zombie betgeniusd processes after test failure, you can kill them
by running the following commands. **Note that these commands will kill all
betgeniusd processes running on the system, so should not be used if any non-test
betgeniusd processes are being run.**
...
sure betgeniusd processes are stopped as above):
...
- when run by [our CI (Continuous Integration)](/ci/README.md), no logs are output to the console. However, if a test
  fails, the `test_framework.log` and betgeniusd `debug.log`s will all be dumped
...
`test_framework.log` and betgeniusd `debug.log`s can be combined into a single
...
call methods that interact with the betgeniusd nodes-under-test.

If further introspection of the betgeniusd instances themselves becomes
...
```bash
cat /tmp/user/1000/testo9vsdjo3/node1/regtest/betgeniusd.pid
```

Then you can use the pid to start `gdb`:

```bash
gdb /home/example/betgeniusd <pid>
```
