...
const char * const BETGENIUS_CONF_FILENAME = "betgenius.conf";
const char * const BETGENIUS_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //betgenius-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", BETGENIUS_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows: C:\Users\Username\AppData\Roaming\BetGenius
    // macOS: ~/Library/Application Support/BetGenius
    // Unix-like: ~/.betgenius
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BetGenius";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/BetGenius";
#else
    // Unix-like
    return pathRet / ".betgenius";
#endif
#endif
}
