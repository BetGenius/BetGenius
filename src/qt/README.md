This directory contains the source code for the BetGenius Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
- Represents the main window of the BetGenius UI.
...
- ClientModel is used by the main application `betgeniusgui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a betgenius:... link from a browser).
...
* UI elements like BetGeniusAmountField, which inherit from QWidget.
* `betgeniusstrings.cpp`: automatically generated
* `betgeniusunits.(h/cpp)`: BETG / mBETG / etc. handling
* `callback.h`
...
**Note:** Do not change `local/betgenius_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `betgenius-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `betgenius-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `betgenius-qt`
9. You're all set! Start developing, building, and debugging the BetGenius Core GUI
