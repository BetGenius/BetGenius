...
#include <qt/betgeniusaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a BetGenius address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new BetGeniusAddressEntryValidator(parent));
    widget->setCheckValidator(new BetGeniusAddressCheckValidator(parent));
}
...
bool parseBetGeniusURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no betgenius: URI
    if(!uri.isValid() || uri.scheme() != QString("betgenius"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!BetGeniusUnits::parse(BetGeniusUnit::BETG, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseBetGeniusURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseBetGeniusURI(uriInstance, out);
}

QString formatBetGeniusURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("betgenius:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(BetGeniusUnits::format(BetGeniusUnit::BETG, info.amount, false, BetGeniusUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openBetGeniusConf()
{
    ...
    /* Open betgenius.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == ChainType::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "BetGenius.lnk";
    if (chain == ChainType::TESTNET) // Remove this special case when testnet CBaseChainParams::DataDir() is incremented to "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "BetGenius (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("BetGenius (%s).lnk", ChainTypeToString(chain)));
}

bool GetStartOnSystemStartup()
{
    // check for BetGenius*.lnk
...
fs::path static GetAutostartFilePath()
{
    ChainType chain = gArgs.GetChainType();
    if (chain == ChainType::MAIN)
        return GetAutostartDir() / "betgenius.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("betgenius-%s.desktop", ChainTypeToString(chain)));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        ChainType chain = gArgs.GetChainType();
        // Write a betgenius.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == ChainType::MAIN)
            optionFile << "Name=BetGenius\n";
        else
            optionFile << strprintf("Name=BetGenius (%s)\n", ChainTypeToString(chain));
