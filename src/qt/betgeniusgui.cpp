...
const std::string BetGeniusGUI::DEFAULT_UIPLATFORM =
...
BetGeniusGUI::BetGeniusGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    if(enableWallet)
    {
        ...
        connect(walletFrame, &WalletFrame::createWalletButtonClicked, this, &BetGeniusGUI::createWallet);
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &BetGeniusGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &BetGeniusGUI::showModalOverlay);
...
void BetGeniusGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a BetGenius address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and betgenius: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &BetGeniusGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &BetGeniusGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &BetGeniusGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your BetGenius addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified BetGenius addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed BetGenius Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed BetGenius Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a betgenius: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible BetGenius command-line options").arg(PACKAGE_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &BetGeniusGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &BetGeniusGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &BetGeniusGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &BetGeniusGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &BetGeniusGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &BetGeniusGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const std::pair<const std::string, bool>& i : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &BetGeniusGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &BetGeniusGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, this, &BetGeniusGUI::createWallet);
        connect(m_close_all_wallets_action, &QAction::triggered, [this] {
            m_wallet_controller->closeAllWallets(this);
        });
        connect(m_migrate_wallet_action, &QAction::triggered, [this] {
            auto activity = new MigrateWalletActivity(m_wallet_controller, this);
            connect(activity, &MigrateWalletActivity::migrated, this, &BetGeniusGUI::setCurrentWallet);
            activity->migrate(walletFrame->currentWalletModel());
        });
        connect(m_mask_values_action, &QAction::toggled, this, &BetGeniusGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &BetGeniusGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &BetGeniusGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &BetGeniusGUI::showDebugWindow);
}

void BetGeniusGUI::createMenuBar()
{
    appMenuBar = menuBar();
...
void BetGeniusGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &BetGeniusGUI::setCurrentWalletBySelectorIndex);
...
void BetGeniusGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &BetGeniusGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &BetGeniusGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &BetGeniusGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &BetGeniusGUI::showProgress);
...
#ifdef ENABLE_WALLET
void BetGeniusGUI::enableHistoryAction(bool privacy)
{
...
void BetGeniusGUI::setWalletController(WalletController* wallet_controller, bool show_loading_minimized)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &BetGeniusGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &BetGeniusGUI::removeWallet);
...
WalletController* BetGeniusGUI::getWalletController()
{
    return m_wallet_controller;
}

void BetGeniusGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &BetGeniusGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &BetGeniusGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &BetGeniusGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &BetGeniusGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &BetGeniusGUI::incomingTransaction);
    connect(this, &BetGeniusGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void BetGeniusGUI::removeWallet(WalletModel* walletModel)
{
...
void BetGeniusGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void BetGeniusGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void BetGeniusGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void BetGeniusGUI::setWalletActionsEnabled(bool enabled)
{
...
void BetGeniusGUI::createTrayIcon()
{
...
void BetGeniusGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &BetGeniusGUI::toggleHidden);
...
void BetGeniusGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void BetGeniusGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void BetGeniusGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void BetGeniusGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void BetGeniusGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void BetGeniusGUI::openClicked()
{
...
void BetGeniusGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void BetGeniusGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void BetGeniusGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void BetGeniusGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void BetGeniusGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void BetGeniusGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void BetGeniusGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void BetGeniusGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to BetGenius network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void BetGeniusGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void BetGeniusGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void BetGeniusGUI::updateHeadersSyncProgressLabel()
{
...
void BetGeniusGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void BetGeniusGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &BetGeniusGUI::quitRequested);
...
void BetGeniusGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void BetGeniusGUI::createWallet()
{
#ifdef ENABLE_WALLET
#ifndef USE_SQLITE
    ...
    connect(activity, &CreateWalletActivity::created, this, &BetGeniusGUI::setCurrentWallet);
    connect(activity, &CreateWalletActivity::created, rpcConsole, &RPCConsole::setCurrentWallet);
    activity->create();
#endif // ENABLE_WALLET
}

void BetGeniusGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void BetGeniusGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &BetGeniusGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &BetGeniusGUI::show);
                e->ignore();
            }
        }
    }
...
void BetGeniusGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void BetGeniusGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void BetGeniusGUI::incomingTransaction(const QString& date, BetGeniusUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(BetGeniusUnits::formatWithUnit(unit, amount, true));
...
void BetGeniusGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void BetGeniusGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool BetGeniusGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool BetGeniusGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void BetGeniusGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void BetGeniusGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void BetGeniusGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void BetGeniusGUI::updateProxyIcon()
{
    std::string ip_port;
...
void BetGeniusGUI::updateWindowTitle()
{
...
void BetGeniusGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void BetGeniusGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void BetGeniusGUI::detectShutdown()
{
...
void BetGeniusGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void BetGeniusGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(BetGeniusGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = BetGeniusGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void BetGeniusGUI::subscribeToCoreSignals()
{
...
void BetGeniusGUI::unsubscribeFromCoreSignals()
{
...
bool BetGeniusGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<BetGeniusUnit> units = BetGeniusUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const BetGeniusUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, BetGeniusUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const BetGeniusUnit u : BetGeniusUnits::availableUnits()) {
        menu->addAction(BetGeniusUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(BetGeniusUnit newUnits)
{
    setText(BetGeniusUnits::longName(newUnits));
}
