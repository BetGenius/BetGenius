...
/** Call node.updateRwSetting() with BetGenius 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because BetGenius 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/BetGenius/BetGenius/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because betgenius 22.x will not try to read these.
...
//! Convert enabled/size values to betgenius -prune setting.
...
//! Get pruning enabled value to show in GUI from betgenius -prune setting.
...
//! Get pruning size value to show in GUI from betgenius -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayBetGeniusUnit")) {
        settings.setValue("DisplayBetGeniusUnit", QVariant::fromValue(BetGeniusUnit::BETG));
    }
    QVariant unit = settings.value("DisplayBetGeniusUnit");
    if (unit.canConvert<BetGeniusUnit>()) {
        m_display_betgenius_unit = unit.value<BetGeniusUnit>();
    } else {
        m_display_betgenius_unit = BetGeniusUnit::BETG;
        settings.setValue("DisplayBetGeniusUnit", QVariant::fromValue(m_display_betgenius_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if betgenius.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_betgenius_unit);
...
{
    if (new_unit.isNull() || new_unit.value<BetGeniusUnit>() == m_display_betgenius_unit) return;
    m_display_betgenius_unit = new_unit.value<BetGeniusUnit>();
    QSettings settings;
    settings.setValue("DisplayBetGeniusUnit", QVariant::fromValue(m_display_betgenius_unit));
    Q_EMIT displayUnitChanged(m_display_betgenius_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/BetGenius/BetGenius/pull/8273
    ...
    // (https://github.com/betgenius-core/gui/issues/567).
