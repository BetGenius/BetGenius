...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<BetGeniusUnit>("BetGeniusUnit");
#else
    qRegisterMetaType<BetGeniusUnit>("BetGeniusUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. betgenius_de.qm (shortcut "de" needs to be defined in betgenius.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. betgenius_de_DE.qm (shortcut "de_DE" needs to be defined in betgenius.qrc)
...
static const char* qt_argv = "betgenius-qt";

BetGeniusApplication::BetGeniusApplication()
...
void BetGeniusApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the BetGeniusApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", BetGeniusGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void BetGeniusApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool BetGeniusApplication::createOptionsModel(bool resetSettings)
{
...
void BetGeniusApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new BetGeniusGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &BetGeniusGUI::quitRequested, this, &BetGeniusApplication::requestShutdown);
...
void BetGeniusApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void BetGeniusApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool BetGeniusApplication::baseInitialize()
{
    return node().baseInitialize();
}

void BetGeniusApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &BetGeniusApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &BetGeniusApplication::handleRunawayException);
    connect(this, &BetGeniusApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &BetGeniusApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void BetGeniusApplication::parameterSetup()
{
...
void BetGeniusApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void BetGeniusApplication::requestInitialize()
{
...
void BetGeniusApplication::requestShutdown()
{
...
void BetGeniusApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        // betgenius: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &BetGeniusGUI::handlePaymentRequest);
            connect(window, &BetGeniusGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void BetGeniusApplication::handleRunawayException(const QString &message)
{
...
void BetGeniusApplication::handleNonFatalException(const QString& message)
{
...
WId BetGeniusApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool BetGeniusApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", BetGeniusGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(betgenius);
    Q_INIT_RESOURCE(betgenius_locale);
    ...
    for (int i = 1; i < argc; i++) {
...
#ifdef ENABLE_WALLET
        if (arg.startsWith(BETGENIUS_IPC_PREFIX, Qt::CaseInsensitive)) {
            invalid_token &= false;
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see betgenius-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, PACKAGE_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see betgenius-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse betgenius.conf, determine network, switch to network specific
    ...
    // betgenius: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
