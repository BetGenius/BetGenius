...
const std::string EXAMPLE_ADDRESS[2] = {"btg1qfhgq8hckew040srz5d5ddqqfve2v4hfwqnuyd3", "btg1qu9c7ex22j4mx2qcu85nnr9vrpwhwrzd59x08y3"};
...
CFeeRate ParseFeeRate(const UniValue& json)
{
    ...
    if (val >= COIN) throw JSONRPCError(RPC_INVALID_PARAMETER, "Fee rates larger than or equal to 1BETG/kvB are not accepted");
    return CFeeRate{val};
}
...
{
    return "> betgenius-cli " + methodname + " " + args + "\n";
}

std::string HelpExampleCliNamed(const std::string& methodname, const RPCArgList& args)
{
    std::string result = "> betgenius-cli -named " + methodname;
...
std::string HelpExampleRpc(const std::string& methodname, const std::string& args)
{
    return "> curl --user myusername --data-binary '{\"jsonrpc\": \"1.0\", \"id\": \"curltest\", "
        "\"method\": \"" + methodname + "\", \"params\": [" + args + "]}' -H 'content-type: text/plain;' http://127.0.0.1:5875/\n";
}
...
std::string HelpExampleRpcNamed(const std::string& methodname, const RPCArgList& args)
{
    ...
    return "> curl --user myusername --data-binary '{\"jsonrpc\": \"1.0\", \"id\": \"curltest\", "
           "\"method\": \"" + methodname + "\", \"params\": " + params.write() + "}' -H 'content-type: text/plain;' http://127.0.0.1:5875/\n";
}
