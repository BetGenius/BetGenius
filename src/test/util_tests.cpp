...
BOOST_AUTO_TEST_CASE(util_ParseMoney)
{
    ...
    BOOST_CHECK(!ParseMoney("25000000000.00"));
...
BOOST_AUTO_TEST_CASE(test_LocaleIndependentAtoi)
{
    ...
    // Ensure legacy compatibility with previous versions of BetGenius Core's atoi64
    for (const auto& pair : atoi64_test_pairs) {
...
BOOST_AUTO_TEST_CASE(test_Capitalize)
{
    ...
    BOOST_CHECK_EQUAL(Capitalize("betgenius"), "Betgenius");
...
BOOST_AUTO_TEST_CASE(message_sign)
{
    const std::array<unsigned char, 32> privkey_bytes = {
        ...
        // just some random data
        // derived address from this private key: BZNuCKjx6QoBdVceTCKGiWtTxoq4YQsYgg
        0xEF, 0x09, 0xB1, 0xB9, 0x68, 0xE7, 0x49, 0x25,
        0x01, 0xF7, 0xA6, 0x10, 0xAF, 0x7F, 0x74, 0xDA,
        0xE7, 0xB3, 0xA9, 0x62, 0x9E, 0xAE, 0x02, 0xFE,
        0x1A, 0xF5, 0xE4, 0x90, 0x0B, 0xFD, 0x7A, 0xEE
    };

    const std::string message = "Trust no one";

    const std::string expected_signature =
        "H5Kk0zPOQikdsZpefp3pQFt/3Qtuds13lMf5VuXlkFlrMdQxgfMQUQ4stCFX6gKXOlvx2y5/G0W7Hr8MhwmHFoU=";
...
BOOST_AUTO_TEST_CASE(message_verify)
{
    ...
    BOOST_CHECK_EQUAL(
        MessageVerify(
            "GJnGACdjRLhbtbKwhHuAXjUbVC52mxXHLN",
            "signature should be irrelevant",
            "message too"),
    ...
    BOOST_CHECK_EQUAL(
        MessageVerify(
            "Bi7r64mzyW6GCg9Dcr661tmFBeZueTcDEd",
    ...
    BOOST_CHECK_EQUAL(
        MessageVerify(
            "BZNuCKjx6QoBdVceTCKGiWtTxoq4YQsYgg",
            "HyjLgF/RG/+iyTARpX87XM3Ef7ey+5b5bWULDYkni3dRUdlprDBAI/LWiDziFvEf5F+omn6Hp5vArNfKNL9vjnQ=",
            "I never signed this"),
    ...
    BOOST_CHECK_EQUAL(
        MessageVerify(
            "BZNuCKjx6QoBdVceTCKGiWtTxoq4YQsYgg",
            "IM9Fxtg5MObmw3Z3gNeXRo0PHpJmFQj/WdmoEEBtnqYbF2VTxYJSduzZYnL+kROaS9bts8hiGD5xuqx9erMSskk=",
            "Trust me"),
        MessageVerificationResult::OK);
}
...
BOOST_AUTO_TEST_CASE(util_WriteBinaryFile)
{
    ...
    std::string expected_text = "betgenius";
