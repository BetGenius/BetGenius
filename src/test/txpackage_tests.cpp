...
BOOST_FIXTURE_TEST_CASE(package_validation_tests, TestChain100Setup)
{
    ...
    auto mtx_parent = CreateValidMempoolTransaction(/*input_transaction=*/m_coinbase_txns[0], /*input_vout=*/0,
                                                    ...
                                                    /*output_amount=*/CAmount(2099999 * COIN), /*submit=*/false);
    ...
    auto mtx_child = CreateValidMempoolTransaction(/*input_transaction=*/tx_parent, /*input_vout=*/0,
                                                   ...
                                                   /*output_amount=*/CAmount(2099998 * COIN), /*submit=*/false);
...
BOOST_FIXTURE_TEST_CASE(noncontextual_package_tests, TestChain100Setup)
{
    ...
    // 24 Parents and 1 Child
    {
        ...
        for (int i{0}; i < 24; ++i) {
            auto parent = MakeTransactionRef(CreateValidMempoolTransaction(m_coinbase_txns[i + 1],
                                             0, 0, coinbaseKey, spk, CAmount(2099998 * COIN), false));
            package.emplace_back(parent);
            child.vin.emplace_back(COutPoint(parent->GetHash(), 0));
        }
        child.vout.emplace_back(2099997 * COIN, spk2);
    ...
    // 2 Parents and 1 Child where one parent depends on the other.
    {
        ...
        mtx_parent.vout.emplace_back(1260000 * COIN, spk);
        mtx_parent.vout.emplace_back(1260000 * COIN, spk2);
        CTransactionRef tx_parent = MakeTransactionRef(mtx_parent);

        CMutableTransaction mtx_parent_also_child;
        mtx_parent_also_child.vin.emplace_back(COutPoint(tx_parent->GetHash(), 0));
        mtx_parent_also_child.vout.emplace_back(1260000 * COIN, spk);
        ...
        mtx_child.vout.emplace_back(1638000 * COIN, spk);
        CTransactionRef tx_child = MakeTransactionRef(mtx_child);
...
BOOST_FIXTURE_TEST_CASE(package_submission_tests, TestChain100Setup)
{
    ...
    auto mtx_grandchild = CreateValidMempoolTransaction(/*input_transaction=*/tx_child, /*input_vout=*/0,
                                                       ...
                                                       /*output_amount=*/CAmount(2099997 * COIN), /*submit=*/false);
...
BOOST_FIXTURE_TEST_CASE(package_witness_swap_tests, TestChain100Setup)
{
    ...
    mtx_child1.vout[0].nValue = CAmount(2099998 * COIN);
    ...
    auto mtx_parent1 = CreateValidMempoolTransaction(/*input_transaction=*/m_coinbase_txns[1], /*input_vout=*/0,
                                                     ...
                                                     /*output_amount=*/CAmount(2099999 * COIN), /*submit=*/true);
    ...
    mtx_parent2_v1.vout[0].nValue = CAmount(2099998 * COIN);
    ...
    auto mtx_parent3 = CreateValidMempoolTransaction(/*input_transaction=*/m_coinbase_txns[3], /*input_vout=*/0,
                                                     /*input_height=*/0, /*input_signing_key=*/coinbaseKey,
                                                     /*output_destination=*/acs_spk,
                                                     /*output_amount=*/CAmount(2100000 * COIN - low_fee_amt), /*submit=*/false);
    ...
    mtx_mixed_child.vout.emplace_back((2099998 + 2099999 + 2100000 - 1) * COIN, mixed_child_spk);
...
BOOST_FIXTURE_TEST_CASE(package_cpfp_tests, TestChain100Setup)
{
    ...
    const CAmount coinbase_value{2100000 * COIN};
    ...
    // Now that the child's fees have "increased" by 1 BETG, the cheap package should succeed.
    {
    ...
    // Parent pays 1 BETG and child pays none. The parent should be accepted without the child.
    {
        BOOST_CHECK_EQUAL(m_node.mempool->size(), expected_pool_size);
