# User-space, Statically Defined Tracing (USDT) for BetGenius Core

BetGenius Core includes statically defined tracepoints to allow for more
...
**Note**: BetGenius Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to BetGenius Core
...
Multiple tools can list the available tracepoints in a `betgeniusd` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in BetGenius Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/betgeniusd
stap net        outbound_message 0x0000000000107c05 /src/betgeniusd
stap validation block_connected  0x00000000002fb10c /src/betgeniusd
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in BetGenius Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/betgeniusd | grep NT_STAPSDT -A 4 -B 2
