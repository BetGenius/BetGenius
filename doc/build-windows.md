...
Below are some notes on how to build BetGenius Core for Windows.

The options known to work for building BetGenius Core on Windows are:
...
Note that for WSL the BetGenius Core source path MUST be somewhere in the default mount file system, for
example /usr/src/betgenius, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\betgenius`, for example:

    make install DESTDIR=/mnt/c/workspace/betgenius
