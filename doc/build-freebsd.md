...
This guide describes how to build betgeniusd, command-line utilities, and GUI on FreeBSD.
...
### 2. Clone BetGenius Repo
Now that `git` and all the required dependencies are installed, let's clone the BetGenius Core repository to a directory. All build scripts and commands will run from this directory.
``` bash
...
#### Wallet Dependencies
It is not necessary to build wallet functionality to run either `betgeniusd` or `betgenius-qt`.
...
```
to: /path/to/betgenius/depends/x86_64-unknown-freebsd[release-number]
```
...
BetGenius Core includes a GUI built with the cross-platform Qt Framework. To compile the GUI, we need to install `qt5`. Skip if you don't intend to use the GUI.
```bash
pkg install qt5
```
###### libqrencode
...
BetGenius Core can provide notifications via ZeroMQ. If the package is installed, support will be compiled in.
```bash
pkg install libzmq4
```
