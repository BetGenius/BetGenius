# CJDNS support in BetGenius Core

It is possible to run BetGenius Core over CJDNS, an encrypted IPv6 network that
...
done outside of BetGenius Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the BetGenius network and individual nodes.
...
before it will work with your BetGenius Core node. This is described in steps
...
## Run BetGenius Core with CJDNS

Once you are connected to the CJDNS network, the following BetGenius Core
...
When enabled, this option tells BetGenius Core that it is running in an
...
IPv6 local network. This helps BetGenius Core perform better address management:
...
CJDNS support was added to BetGenius Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `betgenius-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/BetGenius/BetGenius/blob/master/doc/tor.md) for
details.

## CJDNS-related information in BetGenius Core

There are several ways to see your CJDNS address in BetGenius Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `betgenius-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `betgenius-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `betgenius-cli help getnodeaddresses` for details.
