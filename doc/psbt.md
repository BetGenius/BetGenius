# PSBT Howto for BetGenius Core

Since BetGenius Core 0.17, an RPC interface exists for Partially Signed BetGenius
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/betgenius/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for BetGenius transactions that are not fully signed
yet, together with relevant metadata to help entities work towards signing it.
It is intended to simplify workflows where multiple parties need to cooperate to
produce a transaction. Examples include hardware wallets, multisig setups, and
[CoinJoin](https://betgeniustalk.org/?topic=279249) transactions.

### Overall workflow

Overall, the construction of a fully signed BetGenius transaction goes through the
following steps:
...
- An **Extractor** produces a valid BetGenius transaction (in network format)
...
#### Multisig with multiple BetGenius Core instances
...
BetGenius Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `betgenius-cli` is used.
...
Later, when *V* BETG has been received on *Amulti*, and Bob and Carol want to
