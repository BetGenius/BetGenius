...
""" Example logging BetGenius Core mempool events using the mempool:added,
...
def main(betgeniusd_path):
    betgeniusd_with_usdts = USDT(path=str(betgeniusd_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    betgeniusd_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    betgeniusd_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    betgeniusd_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    betgeniusd_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[betgeniusd_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/betgeniusd")
        exit(1)
