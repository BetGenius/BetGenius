...
"""Script for verifying BetGenius Core release binaries.

This script attempts to download the sum file SHA256SUMS and corresponding
signature file SHA256SUMS.asc from betgenius.cc and betgenius.cc and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/betgenius-core/guix.sigs/tree/main/builder-keys
...
HOST1 = "https://betgenius.cc"
HOST2 = "https://betgenius.cc"
VERSIONPREFIX = "betgenius-core-"
...
def verify_shasums_signature(
   ...
   ReturnCode, list[SigData], list[SigData], list[SigData], list[SigData]
    ...
    # which pubkeys within the BetGenius community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"betgenius_verify_binaries.{args.version}"
    ...
    # remove binaries that are known not to be hosted by betgenius.cc
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the betgenius release to download; of the format '
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes betgenius.cc lags behind betgenius.cc.)')
    )
