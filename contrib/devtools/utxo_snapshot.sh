...
if (( $# < 3 )); then
  echo 'Usage: utxo_snapshot.sh <generate-at-height> <snapshot-out-path> <betgenius-cli-call ...>'
  echo
  ...
  echo 'Examples:'
  echo
  echo "  ./contrib/devtools/utxo_snapshot.sh 570000 utxo.dat ./src/betgenius-cli -datadir=\$(pwd)/testdata"
  echo '  ./contrib/devtools/utxo_snapshot.sh 570000 - ./src/betgenius-cli'
  exit 1
fi
...
BETGENIUS_CLI_CALL="${*} -rpcclienttimeout=9999999"

# Check if the node is pruned and get the pruned block height
PRUNED=$( ${BETGENIUS_CLI_CALL} getblockchaininfo | awk '/pruneheight/ {print $2}' | tr -d ',' )
...
function cleanup {
  ...
  ${BETGENIUS_CLI_CALL} reconsiderblock "${PIVOT_BLOCKHASH}"

  if $NETWORK_DISABLED; then
    (>&2 echo "Restoring network activity")
    ${BETGENIUS_CLI_CALL} setnetworkactive true
  fi
}
...
if [[ "$REPLY" =~ ^[Yy]*$ || -z "$REPLY" ]]; then
  ...
  ${BETGENIUS_CLI_CALL} setnetworkactive false
else
  (>&2 echo "Network activity remains enabled")
fi

# Block we'll invalidate/reconsider to rewind/fast-forward the chain.
PIVOT_BLOCKHASH=$($BETGENIUS_CLI_CALL getblockhash $(( GENERATE_AT_HEIGHT + 1 )) )
...
${BETGENIUS_CLI_CALL} invalidateblock "${PIVOT_BLOCKHASH}"

if [[ "${OUTPUT_PATH}" = "-" ]]; then
  (>&2 echo "Generating txoutset info...")
  ${BETGENIUS_CLI_CALL} gettxoutsetinfo | grep hash_serialized_3 | sed 's/^.*: "\(.\+\)\+",/\1/g'
else
  (>&2 echo "Generating UTXO snapshot...")
  ${BETGENIUS_CLI_CALL} dumptxoutset "${OUTPUT_PATH}"
fi
