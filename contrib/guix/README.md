# Bootstrappable BetGenius Core Builds

This directory contains the files necessary to perform bootstrappable BetGenius
Core builds.
...
To build BetGenius Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      BetGenius Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/betgenius-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/betgenius-core/guix.sigs).
